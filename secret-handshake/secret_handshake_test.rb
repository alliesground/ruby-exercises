...
class SecretHandshakeTest < MiniTest::Unit::TestCase
  def test_parse_fixnum
    ...
    assert_equal '10', handshake.binary
  end

  def test_parse_non_binary_string
    handshake = SecretHandshake.new('non-binary')
    assert_equal nil, handshake.binary
  end

  def test_parse_binary_string
    handshake = SecretHandshake.new('100011')
    assert_equal '100011', handshake.binary
  end
